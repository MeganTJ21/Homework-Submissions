...
sum(residuals(mod)^2)
deviance(mod)
sum(residuals(mod)^2) == deviance(mod)
```
...
summary(mod)$sigma
sqrt(deviance(mod)/df.residual(mod)) == summary(mod)$sigma
```
...
```
```
Using the function with the `var.equal` argument set to TRUE matches the model summary p-value
```
